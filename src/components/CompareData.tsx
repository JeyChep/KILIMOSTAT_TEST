...
import { X, Plus, BarChart2, Table2, TrendingUp, Download, ChevronDown, Loader2, RefreshCw } from 'lucide-react';
import { apiService, Domain, KilimoDataRecord } from '../services/apiService';
...
export default function CompareData() {
  ...
  const fetchComparison = useCallback(async () => {
    if (!selectedDomain || selectedCounties.length === 0) return;
    ...
    try {
      ...
      const subdomains = await apiService.getSubdomainsByDomain(selectedDomain.id);
      ...
      const countyNameMap = new Map(selectedCounties.map(c => [c.name.toLowerCase(), c]));

      const aggregated = new Map<string, Map<number, number>>();
      for (const c of selectedCounties) {
        aggregated.set(c.name.toLowerCase(), new Map());
      }
  ...
  }, [selectedDomain, selectedCounties, yearFrom, yearTo]);
  ...
  const filteredCounties = allCounties.filter(
    c =>
      ...
      !selectedCounties.find(s => s.id === c.id)
  );
  ...
  const canFetch = selectedCounties.length > 0 && selectedDomain !== null;
  const hasData = seriesData.length > 0 && years.length > 0;

  const allMax = hasData
    ? Math.max(...seriesData.flatMap(s => s.values), 1)
    : 1;

  const latestValues = hasData
    ? seriesData.map(s => ({ county: s.county, value: s.values[s.values.length - 1] || 0, color: s.color }))
    : [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Compare agricultural and economic indicators across counties, domains, and time periods.
          </p>
        </div>

        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Configure Comparison</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="lg:col-span-2">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {selectedCounties.map((county, idx) => (
                  ...
                  >
                    {county.name}
                    <button onClick={() => removeCounty(county.id)} className="ml-1 hover:opacity-75">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                ))}
              ...
              <div className="relative">
                <div
                  className="flex items-center border border-gray-300 rounded-lg px-3 py-2 cursor-text focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500"
                  onClick={() => setShowCountyDropdown(true)}
                >
              ...
              {showCountyDropdown && (
                <div className="fixed inset-0 z-10" onClick={() => setShowCountyDropdown(false)} />
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <button
                  className="w-full flex items-center justify-between border border-gray-300 rounded-lg px-3 py-2 text-sm hover:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-white"
                  ...
                  <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg">
                    <div className="p-2 border-b border-gray-100">
                      <input
                        ...
                        className="w-full text-sm border border-gray-200 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-blue-500"
                        autoFocus
                      />
                    </div>
                    <div className="max-h-48 overflow-y-auto">
                      {filteredDomains.map(domain => (
                        <button
                          key={domain.id}
                          className={`w-full text-left px-4 py-2 text-sm hover:bg-blue-50 hover:text-blue-700 transition-colors ${selectedDomain?.id === domain.id ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700'}`}
                          onMouseDown={() => { setSelectedDomain(domain); setShowDomainDropdown(false); setDomainSearch(''); }}
                        >
                          {domain.name}
                        </button>
                      ))}
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="flex-1 border border-gray-300 rounded-lg px-2 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                >
                  {yearOptions.map(y => (
          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="text-xs text-gray-500">
              {canFetch
                ? `Comparing ${selectedCounties.length} ${selectedCounties.length === 1 ? 'county' : 'counties'} • ${selectedDomain?.name} • ${yearFrom}–${yearTo}`
                : 'Select at least one county and a domain to compare'}
            </div>
            <button
        ...
        {hasData && !loading && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => exportCSV(seriesData, years, selectedDomain)}
            ...
            {viewMode === 'line' && (
              <LineCompareChart series={seriesData} years={years} domain={selectedDomain} allMax={allMax} />
            )}
            {viewMode === 'bar' && (
              <BarCompareChart latestValues={latestValues} barMax={barMax} domain={selectedDomain} latestYear={years[years.length - 1]} />
            )}
            {viewMode === 'table' && (
              <TableView series={seriesData} years={years} domain={selectedDomain} />
            )}

            <SummaryCards series={seriesData} years={years} domain={selectedDomain} />
          </div>
        )}
        ...
        {!hasData && !loading && !canFetch && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-16 text-center">
            ...
            <p className="text-gray-400 text-sm">Choose counties, a domain, and a year range above to get started</p>
          </div>
        )}
      </div>
    </div>
...
function ViewButton({ active, onClick, icon: Icon, label }: { active: boolean; onClick: () => void; icon: React.ElementType; label: string }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${active ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'}`}
    >
...
function LineCompareChart({ series, years, domain, allMax }: { series: CountySeriesData[]; years: number[]; domain: Domain | null; allMax: number }) {
  if (series.length === 0 || years.length === 0) return null;

  const W = 100;
  const H = 80;
  const PAD = { top: 5, right: 2, bottom: 12, left: 8 };

  const innerW = W - PAD.left - PAD.right;
  const innerH = H - PAD.top - PAD.bottom;

  function toX(i: number) {
    return PAD.left + (years.length <= 1 ? innerW / 2 : (i / (years.length - 1)) * innerW);
  }
  function toY(v: number) {
    return PAD.top + innerH - (allMax === 0 ? 0 : (v / allMax) * innerH);
  }

  const gridLines = [0, 0.25, 0.5, 0.75, 1].map(f => allMax * f);

  const labelYears = years.length <= 10
    ? years
    : years.filter((_, i) => i === 0 || i === years.length - 1 || i % Math.ceil(years.length / 5) === 0);

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-900">{domain?.name} — Trend Over Time</h3>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: '320px' }} preserveAspectRatio="none">
        {gridLines.map((g, i) => (
          <g key={i}>
            <line
              ...
              x2={W - PAD.right} y2={toY(g)}
              stroke="#e5e7eb" strokeWidth="0.3"
            />
            <text x={PAD.left - 1} y={toY(g) + 0.8} textAnchor="end" fontSize="2.5" fill="#9ca3af">
              {formatNumber(g)}
            </text>
          </g>
        ))}
        ...
        {labelYears.map((yr) => {
          ...
          return (
            <text key={yr} x={toX(i)} y={H - 1} textAnchor="middle" fontSize="2.5" fill="#9ca3af">
              {yr}
            </text>
          );
        })}
          ...
          return (
            <g key={s.county.id}>
              ...
              {s.values.map((v, i) => v > 0 && (
                <circle key={i} cx={toX(i)} cy={toY(v)} r="0.9" fill={s.color} />
              ))}
            </g>
          );
        })}
...
function BarCompareChart({ latestValues, barMax, domain, latestYear }: { latestValues: { county: County; value: number; color: string }[]; barMax: number; domain: Domain | null; latestYear: number }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-900">{domain?.name} — County Comparison</h3>
        <p className="text-sm text-gray-500">Latest available year: {latestYear}</p>
      </div>
            ...
            <div key={county.id}>
              ...
              <div className="w-full bg-gray-100 rounded-full h-5 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${barMax > 0 ? (value / barMax) * 100 : 0}%`,
...
function TableView({ series, years, domain }: { series: CountySeriesData[]; years: number[]; domain: Domain | null }) {
  const displayYears = years.length > 20
    ? years.filter((_, i) => i % Math.ceil(years.length / 20) === 0 || i === years.length - 1)
    : years;

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100">
        <h3 className="text-lg font-semibold text-gray-900">{domain?.name} — Data Table</h3>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50">
              ...
              {displayYears.map(yr => (
                <th key={yr} className="px-4 py-3 text-right font-semibold text-gray-700 whitespace-nowrap">{yr}</th>
              ))}
            </tr>
          </thead>
          <tbody>
...
function SummaryCards({ series, years, domain }: { series: CountySeriesData[]; years: number[]; domain: Domain | null }) {
  if (series.length === 0 || years.length === 0) return null;
      ...
      {cards.map(card => (
        <div key={card.county.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          ...
          <div className="space-y-2">
            <Stat label="Latest value" value={formatNumber(card.latest)} />
            <Stat label="Average" value={formatNumber(Math.round(card.avg))} />
            <Stat label="Peak" value={formatNumber(card.max)} />
            <Stat
              label="Overall trend"
              value={card.trend !== 0 ? `${card.trend > 0 ? '+' : ''}${card.trend.toFixed(1)}%` : '—'}
              valueClass={card.trend > 0 ? 'text-green-600' : card.trend < 0 ? 'text-red-600' : 'text-gray-500'}
            />
...
function exportCSV(series: CountySeriesData[], years: number[], domain: Domain | null) {
  ...
  a.href = url;
  a.download = `compare_${domain?.name ?? 'data'}_${years[0]}_${years[years.length - 1]}.csv`;
  a.click();
